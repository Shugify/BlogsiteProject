...
class Administrator(AbstractBaseUser):  # 管理员信息表
    DoesNotExist = None
    objects = models.Manager()
    # 管理员id，主键
    ...
    # 管理员头像profile photo
    administrator_pfp = models.ImageField(default='app01/static/app01/image/default.jpg')
    # 管理员注册日期
    ...
    # 将 user_id 设置为用户名字段
    USERNAME_FIELD = 'administrator_id'
    # REQUIRED_FIELDS 需要的字段列表，这里不需要，因为我们在 create_user 方法中设置了必需的字段
    REQUIRED_FIELDS = []
...
class Article(models.Model):  # 文章信息表
    ...
    article_adAuthor = models.ForeignKey(Administrator, on_delete=models.SET_NULL, null=True, related_name='ad_article')
    ...
    # 每当需要取出文章列表，作为博客首页时，按照-article_created（即文章创建时间，负号标识倒序）来排列，保证了最新文章永远在最顶部位置。
    class Meta:
...
class Comment(models.Model):  # 评论信息表
    ...
    comment_ad = models.ForeignKey(Administrator, on_delete=models.SET_NULL, null=True, related_name='ad_comment')
    # 接收评论的用户
    comment_receiver = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='comment_receiver')
    # 评论内容
