...
def update_article(request, id):
    ...
    # 获取与本篇文章有关的分类关系
    relations = ArticleCategory.objects.filter(article=article)
    # 获取本篇文章的分类项
    selected_categories = []
    for rel in relations:
        selected_categories.append(rel.category)
    # 过滤非作者的用户
    ...
    if request.method == "POST":
        ...
        if article_post_form.is_valid():
            ...
            if selected_categories:
                # 删除本篇文章原本的分类关系
                the_list = ArticleCategory.objects.filter(article=article)
                for i in the_list:
                    i.delete()
                # 保存修改过的文章对象
                article.save()
...
# 管理员发表评论
def ad_post_comment(request, article_id):
    article = get_object_or_404(Article, article_id=article_id)
    # 获取特定用户
    administrator = models.Administrator.objects.get(administrator_id=request.session['administrator_id'])
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            # 设置评论所属的文章和用户
            new_comment.comment_article = article
            new_comment.comment_ad = administrator
            new_comment.save()  # comment_created 将在此时自动设置为当前时间
            # 可以重定向或渲染相同页面
            return redirect(reverse('ad_article', kwargs={'id': article_id}))
        else:
            return HttpResponse("表单内容有误，请重新填写。")
    # 处理错误请求
    else:
...
# 用户个人主页按时间顺序展示（普通用户看）
def user_detail_lastest(request, id, flag):
    all_views = 0
    all_comment = 0
    count = 0
    if flag:
        # 获取特定管理员
        user = models.Administrator.objects.get(administrator_id=id)
        if user.administrator_sex is None:
            judge_sex = 0
        else:
            judge_sex = 1
            if user.administrator_sex == 1:
                sex = 1
            else:
                sex = 0
        if user.administrator_age is None:
            judge_age = 0
        else:
            judge_age = 1
        # 获取该管理员的所有文章（按时间排序）
        article_list = models.Article.objects.filter(article_adAuthor=user)
        for i in article_list:
            all_views = all_views+i.article_views
            all_comment = all_comment+i.article_commentcnt
            count = count+1
        user_register_date = user.administrator_register  # 假设这是用户注册的日期
        current_date = timezone.now().date()  # 获取当前日期
        days_diff = (current_date - user_register_date).days  # 计算注册到当前的天数
    ...
    # 将导航对象相应的页码内容返回给 articles
    articles = paginator.get_page(page)
    return render(request, 'app01/user_detail_lastest.html', locals())


# 用户个人主页按浏览量展示（普通用户看）
def user_detail_top(request, id, flag):
    all_views = 0
    all_comment = 0
    count = 0
    if flag:
        ...
        if user.administrator_age is None:
            ...
            judge_age = 1
        # 获取该管理员的所有文章（按浏览量排序）
        article_list = models.Article.objects.filter(article_adAuthor=user).order_by('-article_views')[:]
        for i in article_list:
            all_views = all_views+i.article_views
            all_comment = all_comment+i.article_commentcnt
            count = count+1
        ...
        if user.user_age is None:
            ...
            judge_age = 1
        # 获取该用户的所有文章（按浏览量排序）
        article_list = models.Article.objects.filter(article_author=user).order_by('-article_views')[:]
        for i in article_list:
            all_views = all_views + i.article_views
            all_comment = all_comment + i.article_commentcnt
            count = count + 1
    ...
    # 将导航对象相应的页码内容返回给 articles
    articles = paginator.get_page(page)
    return render(request, 'app01/user_detail_top.html', locals())


# 用户个人主页按时间顺序展示（管理员用户看）
def ad_user_detail_lastest(request, id, flag):
    all_views = 0
    all_comment = 0
    count = 0
    if flag:
    ...
    # 将导航对象相应的页码内容返回给 articles
    articles = paginator.get_page(page)
    return render(request, 'app01/ad_user_detail_lastest.html', locals())


# 用户个人主页按浏览量顺序展示（管理员用户看）
def ad_user_detail_top(request, id, flag):
    all_views = 0
    all_comment = 0
    count = 0
    if flag:
    ...
    # 将导航对象相应的页码内容返回给 articles
    articles = paginator.get_page(page)
    return render(request, 'app01/ad_user_detail_top.html', locals())
